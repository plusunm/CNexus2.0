"""Tests for POST dispatch helper."""
...
def _load_modules():
    ...
    dispatch = importlib.util.spec_from_file_location(
        f"{pkg}.http.post_dispatch",
        os.path.join(GATEWAY_DIR, "http", "post_dispatch.py"),
    )
    dispatch_mod = importlib.util.module_from_spec(dispatch)
    dispatch_mod.__package__ = f"{pkg}.http"
    sys.modules[f"{pkg}.http.post_dispatch"] = dispatch_mod
...
class PostDispatchTests(unittest.TestCase):
    ...
    def test_dispatch_first_matching_route(self):
        def route_a(handler, path, query):
            if path == "/api/converse":
                return self.responses_mod.HttpRouteResponse.json({"ok": True, "route": "a"})
            return None

        def route_b(handler, path, query):
            return self.responses_mod.HttpRouteResponse.json({"ok": True, "route": "b"})

        handler = type("H", (), {"_json_calls": [], "_json": lambda s, b, st=200: s._json_calls.append((b, st))})()
        ok = self.dispatch_mod.dispatch_post(handler, "/api/converse", "", [route_a, route_b])
        self.assertTrue(ok)
        self.assertEqual(handler._json_calls[0][0]["route"], "a")

    def test_dispatch_returns_false_when_unhandled(self):
        handler = type("H", (), {})()
        self.assertFalse(self.dispatch_mod.dispatch_post(handler, "/unknown", "", []))
